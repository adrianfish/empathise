...
exports.bareToPathSpecifiersTransform = (query) => stream_js_1.transformStream((file) => __awaiter(this, void 0, void 0, function* () {

  const plugins = [...babel_syntax_plugins_js_1.babelSyntaxPlugins, babel_plugin_bare_specifiers_js_1.resolveBareSpecifiers(file.path, false, undefined, undefined, undefined, query)];
  const scriptSource = yield file_js_1.getFileContents(file);
  try {
    const pathParts = file.path.split('node_modules/');
    const relativePath = pathParts[1];
    console.log(`Applying empathise to ${relativePath}`);
